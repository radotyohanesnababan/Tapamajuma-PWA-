...
return Application::configure(basePath: dirname(__DIR__))
    ...
    // 1. Register LogActivityMiddleware
    $middleware->append(\App\Http\Middleware\LogActivityMiddleware::class);

    // 2. Cukup gunakan satu baris ini (Otomatis menangani Sanctum)
    $middleware->statefulApi();

    // 3. Paksa JSON (Wajib agar tidak Redirect)
    $middleware->append(\App\Http\Middleware\ForceJsonResponse::class);

    // 4. Matikan Redirect Fisik (Penyebab utama Network Error)
